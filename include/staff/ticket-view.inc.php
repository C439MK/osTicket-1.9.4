...
if($ticket->isOverdue())
    $warn.='&nbsp;&nbsp;<span class="Icon overdueTicket">Marked for review!</span>';
...
<table width="940" cellpadding="2" cellspacing="0" border="0">
    <tr>
        ...
        <td width="50%" class="right_align has_bottom_border">
            ...
            <?php } ?>
            <?php
			// This removes the close and reopen button at top of single ticket view
			/*
            ...
            <div id="action-dropdown-more" class="action-dropdown anchor-right">
              <ul>
                ...
                if($ticket->isOpen() && ($dept && $dept->isManager($thisstaff))) {
                    ...
                    if(!$ticket->isOverdue()) { ?>
                        <li><a class="confirm-action" id="ticket-overdue" href="#overdue"><i class="icon-bell"></i> Mark as for Review</a></li>
                    <?php
                    }
...
foreach (DynamicFormEntry::forTicket($ticket->getId()) as $form) {
    ...
    ?>
        ...
        <td colspan="2">
            <table cellspacing="0" cellpadding="4" width="100%" border="0">
            <?php print_r($answers->field->ht['name']['lot']); foreach($answers as $a) {
           ...
           <tbody id="to_sec">
            <tr>
                ...
                <td>
                    ...
                    <select id="emailreply" name="emailreply">
                        ...
                        <option value="0" selected="selected" >&mdash;Do Not Email Reply&mdash;</option>
                    </select>
                </td>
            </tr>
            </tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    if($ticket->isClosed()) { ?>
                        <label><input type="hidden" name="reply_ticket_status" id="reply_ticket_status" value="Open" >
						<label><input checked="checked" disabled type="checkbox" name="" id="" value=""
                    ...
                    <select name="state">						                      
                        ...
                        if($ticket->isClosed()){
                            echo sprintf('<option value="open" %s>Reopen Ticket</option>',
                                    ($state=='reopen')?'selected="selected"':'');
                        } else {
                            if($thisstaff->canCloseTickets())
								echo '<option value="" selected="">&mdash; unchanged &mdash;</option>';
                                echo sprintf('<option value="closed" %s>Close Ticket</option>',
                                    ($state=='closed')?'selected="selected"':'');
                            ...
                            if($dept && $dept->isManager($thisstaff)) {
                                if(!$ticket->isOverdue())
                                    echo sprintf('<option value="overdue" %s>Flag for Review</option>',
                                        ($state=='answered')?'selected="selelected"':'');
                                else
                                    echo sprintf('<option value="notdue" %s>Clear Review Flag</option>',
...
<div style="display:none;" class="dialog" id="confirm-action">
    ...
    <p class="confirm-action" style="display:none;" id="answered-confirm">
        Are you sure want to flag the ticket as <b>acknowledged/actioned</b>?
    </p>
    ...
    <p class="confirm-action" style="display:none;" id="overdue-confirm">
        Are you sure want to flag the ticket for <font color="red"><b>review</b></font>?
    </p>
