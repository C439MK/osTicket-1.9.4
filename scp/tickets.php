    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'reply':
            if(!$thisstaff->canPostReply())
				...
				// add checks for mandatory fields on close here
				if($_POST["reply_ticket_status"] == "Closed") {
					if(empty($ticket->_answers["acc-rej"]->ht["value"]))
						$errors['err']='"Accepted / Rejected" field must be complete before closing.';
					if($ticket->_answers["amount"]->ht["value"] == "") // we have to check for empty string and not empty() because the users can add 0.00 which is considered empty by PHP function empty()
						$errors['err']='"Amount" field must be complete before closing.';
					if(empty($ticket->_answers["resolved"]->ht["value"]))
						$errors['err']='"Date Resolved" field must be complete before closing.';
					if(empty($ticket->_answers["pledge"]->ht["value"]))
						$errors['err']='"Pledge" field must be complete before closing.';						
					if(empty($ticket->_answers["prev"]->ht["value"]))
						$errors['err']='"Prevented" field must be complete before closing.';
				}	
            }
        ...
        case 'postnote': /* Post Internal Note */
            ...
            if($_POST['state']) {
                if($_POST['state']=='closed' && !$thisstaff->canCloseTickets()) {
					...
					if(in_array($_POST['state'], array('overdue', 'notdue', 'unassigned')) && (!($dept=$ticket->getDept()) || !$dept->isManager($thisstaff)))
            ...
            if(($note=$ticket->postNote($vars, $errors, $thisstaff))) {
                ...
                $errors['postnote'] = 'Unable to post the note. Correct the error(s) and try again!';
            }
            break;
        case 'edit':
        case 'update':
            ...
            switch(strtolower($_POST['do'])):
                ...
                case 'overdue':
                    ...
                    if(!$dept || !$dept->isManager($thisstaff)) {
                        $errors['err']='Permission Denied. You are not allowed to flag tickets for review';
                    } elseif($ticket->markOverdue()) {
                        $msg='Ticket flagged as for review';
                        $ticket->logActivity('Ticket Marked for Review',($msg.' by '.$thisstaff->getName()));
                    } else {
                        $errors['err']='Problems marking the ticket for review. Try again';
                    }
                    break;
                case 'answered':
                    $dept = $ticket->getDept();
                    if(!$dept || !$dept->isManager($thisstaff)) {
                        ...
                        $msg='Ticket flagged as Acknowledged/Actioned';
                        $ticket->logActivity('Ticket Marked Acknowledged/Actioned',($msg.' by '.$thisstaff->getName()));
                    } else {
                        $errors['err']='Problems marking the the ticket Acknowledged/Actioned. Try again';
                    }
                    break;
                case 'unanswered':
                    $dept = $ticket->getDept();
                ...
                elseif(!$_POST['tids'] || !is_array($_POST['tids']))
                    ...
                    switch(strtolower($_POST['do'])) {
                        ...
                        case 'close':
                            ...
                            break;
                        case 'mark_overdue':
                            $note='Ticket flagged for review by '.$thisstaff->getName();
                            foreach($_POST['tids'] as $k=>$v) {
                                if(($t=Ticket::lookup($v)) && !$t->isOverdue() && $t->markOverdue()) {
                                    $i++;
                                    $t->logNote('Ticket Marked for Review', $note, $thisstaff);
                                }
                            }

                            if($i==$count)
                                $msg = "Selected tickets ($i) marked for review";
                            elseif($i)
                                $warn = "$i of $count selected tickets marked for review";
                            else
                                $errors['err'] = 'Unable to flag selected tickets for review';
                            break;
                        case 'delete':
                            if($thisstaff->canDeleteTickets()) {
                                foreach($_POST['tids'] as $k=>$v) {
...
if($cfg->showAnsweredTickets()) {
    ...
    if($stats['answered']) {
        $nav->addSubMenu(array('desc'=>'Acknowledged/Actioned ('.number_format($stats['answered']).')',
                               'title'=>'Acknowledged/Actioned Tickets',
...
if($stats['overdue']) {
    $nav->addSubMenu(array('desc'=>'Review ('.number_format($stats['overdue']).')',
                           'title'=>'Stale Tickets',
    ...
    if(!$sysnotice && $stats['overdue']>10)
        $sysnotice=$stats['overdue'] .' tickets for review!';
}
